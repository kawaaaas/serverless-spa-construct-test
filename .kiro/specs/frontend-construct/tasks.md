...
- [ ] 1. FrontendConstructの基本実装
  - [ ] 1.1 FrontendConstructPropsインターフェースを定義する
    - api?: IRestApi（オプション）
  ...
  - [ ] 1.2 FrontendConstructクラスの基本構造を実装する
    - Constructを継承
...
- [ ] 3. CloudFront Functionの実装
  - [ ] 3.1 SPAルーティング用CloudFront Functionを作成する
    - 拡張子なしパス → /index.htmlに書き換え
    - 拡張子ありパス → そのまま通過
    - viewer-requestイベントで実行
    - _要件: 3.1, 3.2, 3.3, 3.4_

- [ ] 4. CloudFrontディストリビューションの実装
  - [ ] 4.1 S3オリジンとOACを設定する
  ...
  - [ ] 4.3 エラーレスポンスを設定する
    ...
    - 404 → /index.html (200)
    - _要件: 3.5, 3.6_
  - [ ] 4.4 distributionPropsによる上書き機能を実装する
    - _要件: 2.5_

- [ ] 5. API Gatewayルーティングの実装（オプション）
  - [ ] 5.1 API Gateway オリジンを設定する（api指定時のみ）
    - HttpOriginを使用
    - カスタムヘッダーを付与
    - _要件: 4.1, 4.2, 5.1_
  - [ ] 5.2 /api/\*ビヘイビアを追加する（api指定時のみ）
...
- [ ] 7. ユニットテスト実装
  ...
  - [ ] 7.2 CloudFrontディストリビューションテストを実装する
    - ディストリビューションが作成されること
    - OACが作成・関連付けされること
...
- [ ] 8. エクスポート設定
  - [ ] 8.1 lib/index.tsにFrontendConstructをエクスポートする
